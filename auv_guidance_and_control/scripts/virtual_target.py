...
class VirtualTarget():
    def __init__(self, Path, AUVModel, ActuatorModel, DPControlSystem, heading_mode='path_dependent_heading', dot_s_bounds=[-1, 1], point=[0,0], current_velocity=[0,0,0,0,0,0]):
        self.path = Path
    ...
    def optimize_along_track_speed(self, eta, nu, dot_eta_c):
        tau_c = lambda dot_s : self.dp_control_system.pd_regulate(eta, nu, self.eta_t, self.nu_t(dot_s), self.dot_nu_t(dot_s), dot_eta_c)
        f = lambda x : -x
